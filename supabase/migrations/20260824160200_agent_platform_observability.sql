...
create table if not exists internal.repository_indexes (
...
create index if not exists repository_indexes_run_created_idx on internal.repository_indexes(run_id,created_at desc);
create index if not exists repository_indexes_resource_revision_idx on internal.repository_indexes(resource_id,revision_sha);

create table if not exists internal.repository_index_nodes (
...
create index if not exists repository_index_nodes_path_idx on internal.repository_index_nodes(index_id,path) where path is not null;

create table if not exists internal.repository_index_edges (
  index_id uuid not null references internal.repository_indexes(id) on delete cascade,
...
create table if not exists internal.impact_analyses (
  id uuid primary key default gen_random_uuid(),
  run_id uuid not null references internal.agent_runs(id) on delete cascade,
  verification_round integer not null check (verification_round >= 0),
  repository_index_id uuid references internal.repository_indexes(id) on delete set null,
...
create index if not exists impact_analyses_run_idx on internal.impact_analyses(run_id,verification_round desc);

create table if not exists internal.impact_nodes (
  analysis_id uuid not null references internal.impact_analyses(id) on delete cascade,
  node_key text not null,
  kind text not null,
  path text,
...
create table if not exists internal.verification_runs (
  id uuid primary key default gen_random_uuid(),
  run_id uuid not null references internal.agent_runs(id) on delete cascade,
  verification_round integer not null check (verification_round >= 0),
  repository_index_id uuid references internal.repository_indexes(id) on delete set null,
  impact_analysis_id uuid references internal.impact_analyses(id) on delete set null,
...
create index if not exists verification_runs_created_idx on internal.verification_runs(created_at desc);

create table if not exists internal.verification_results (
  verification_run_id uuid not null references internal.verification_runs(id) on delete cascade,
...
create table if not exists internal.run_skill_observations (
  run_id uuid not null references internal.agent_runs(id) on delete cascade,
...
alter table internal.repository_index_nodes enable row level security;
alter table internal.repository_index_edges enable row level security;
alter table internal.impact_analyses enable row level security;
alter table internal.impact_nodes enable row level security;
alter table internal.verification_runs enable row level security;
alter table internal.verification_results enable row level security;
alter table internal.run_skill_observations enable row level security;

revoke all on table internal.agent_run_intelligence, internal.repository_indexes, internal.repository_index_nodes, internal.repository_index_edges, internal.impact_analyses, internal.impact_nodes, internal.verification_runs, internal.verification_results, internal.run_skill_observations from public,anon,authenticated;
grant all on table internal.agent_run_intelligence, internal.repository_indexes, internal.repository_index_nodes, internal.repository_index_edges, internal.impact_analyses, internal.impact_nodes, internal.verification_runs, internal.verification_results, internal.run_skill_observations to service_role;
...
begin
  ...
  delete from internal.run_skill_observations where run_id=target_run_id;
  for item in select value from jsonb_array_elements(coalesce(target_skills,'[]'::jsonb)) loop
    position := position + 1;
    insert into internal.run_skill_observations(run_id,skill_key,activation_order)
...
begin
  ...
  insert into internal.repository_indexes(run_id,resource_id,phase,verification_round,repository,ref,revision_sha,content_revision_hash,status,complete,project_profile,counts,finished_at)
  ...
  delete from internal.repository_index_nodes where index_id=result_id;
  delete from internal.repository_index_edges where index_id=result_id;
  return result_id;
end $$;
...
begin
  ...
  if not exists(select 1 from internal.repository_indexes i where i.id=target_index_id and i.status='writing') then raise exception 'repository_index_not_writable'; end if;
  insert into internal.repository_index_nodes(index_id,node_key,kind,path,label,metadata)
  ...
  insert into internal.repository_index_edges(index_id,from_key,to_key,kind,metadata)
...
as $$ begin
  if coalesce(auth.jwt()->>'role','') <> 'service_role' then raise exception 'service_role_required' using errcode='42501'; end if;
  update internal.repository_indexes set status=case when target_complete then 'ready' else 'partial' end,complete=target_complete,counts=coalesce(target_counts,'{}'::jsonb),finished_at=now() where id=target_index_id and status='writing';
...
begin
  ...
  insert into internal.impact_analyses(run_id,verification_round,repository_index_id,risk,changed_count,affected_count,test_count,verification_hints)
  ...
  returning id into result_id;
  delete from internal.impact_nodes where analysis_id=result_id;
  insert into internal.impact_nodes(analysis_id,node_key,kind,path,distance,direction,via)
...
begin
  ...
  insert into internal.verification_runs(run_id,verification_round,repository_index_id,impact_analysis_id,status,plan,blockers,reviewer)
  ...
  returning id into result_id;
  delete from internal.verification_results where verification_run_id=result_id;
  insert into internal.verification_results(verification_run_id,check_kind,required,status,summary,evidence,duration_ms)
...
begin
  ...
  return jsonb_build_object(
    'counts',jsonb_build_object(
      'repository_indexes',(select count(*) from internal.repository_indexes),
      'verification_runs',(select count(*) from internal.verification_runs),
      'verification_passed',(select count(*) from internal.verification_runs where status='passed'),
      'impact_analyses',(select count(*) from internal.impact_analyses),
      'tracked_runs',(select count(*) from internal.agent_run_intelligence)
    ),
    'repository_indexes',coalesce((select jsonb_agg(to_jsonb(x)) from (select i.id,i.run_id,i.phase,i.verification_round,i.repository,i.ref,i.revision_sha,i.status,i.complete,i.counts,i.project_profile,i.created_at,i.finished_at from internal.repository_indexes i order by i.created_at desc limit 20) x),'[]'::jsonb),
    'impacts',coalesce((select jsonb_agg(to_jsonb(x)) from (select a.id,a.run_id,a.verification_round,a.risk,a.changed_count,a.affected_count,a.test_count,a.created_at from internal.impact_analyses a order by a.created_at desc limit 20) x),'[]'::jsonb),
    'verifications',coalesce((select jsonb_agg(to_jsonb(x)) from (select v.id,v.run_id,v.verification_round,v.status,v.blockers,v.reviewer,v.created_at,i.revision_sha,i.ref from internal.verification_runs v left join internal.repository_indexes i on i.id=v.repository_index_id order by v.created_at desc limit 30) x),'[]'::jsonb),
    'skills',coalesce((select jsonb_agg(to_jsonb(x)) from (select s.run_id,s.skill_key,s.activation_order,s.status,s.created_at from internal.run_skill_observations s order by s.created_at desc,s.activation_order asc limit 50) x),'[]'::jsonb)
  );
end $$;
...
begin
  ...
  return jsonb_build_object(
    ...
    'indexes',coalesce((select jsonb_agg(to_jsonb(x)) from (select i.id,i.phase,i.verification_round,i.repository,i.ref,i.revision_sha,i.content_revision_hash,i.status,i.complete,i.project_profile,i.counts,i.created_at,i.finished_at,(select count(*) from internal.repository_index_nodes n where n.index_id=i.id) node_count,(select count(*) from internal.repository_index_edges e where e.index_id=i.id) edge_count from internal.repository_indexes i where i.run_id=target_run_id order by i.created_at) x),'[]'::jsonb),
    'impacts',coalesce((select jsonb_agg(jsonb_build_object('id',a.id,'verification_round',a.verification_round,'risk',a.risk,'changed_count',a.changed_count,'affected_count',a.affected_count,'test_count',a.test_count,'verification_hints',a.verification_hints,'nodes',coalesce((select jsonb_agg(to_jsonb(n)) from internal.impact_nodes n where n.analysis_id=a.id),'[]'::jsonb),'created_at',a.created_at) order by a.verification_round) from internal.impact_analyses a where a.run_id=target_run_id),'[]'::jsonb),
    'verifications',coalesce((select jsonb_agg(jsonb_build_object('id',v.id,'verification_round',v.verification_round,'status',v.status,'plan',v.plan,'blockers',v.blockers,'reviewer',v.reviewer,'results',coalesce((select jsonb_agg(to_jsonb(r) order by r.check_kind) from internal.verification_results r where r.verification_run_id=v.id),'[]'::jsonb),'created_at',v.created_at) order by v.verification_round) from internal.verification_runs v where v.run_id=target_run_id),'[]'::jsonb),
    'skills',coalesce((select jsonb_agg(to_jsonb(x)) from (select s.skill_key,s.activation_order,s.status,s.created_at from internal.run_skill_observations s where s.run_id=target_run_id order by s.activation_order) x),'[]'::jsonb)
  );
end $$;
