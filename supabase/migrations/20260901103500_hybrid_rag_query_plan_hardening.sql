...
begin
  ...
  with semantic as (
    select
      k.id as chunk_id,
      row_number() over(order by e.embedding <=> target_query_embedding, k.id) as semantic_rank
    ...
    order by e.embedding <=> target_query_embedding, k.id
  ...
  select
    ...
    k.content,
    s.provenance,
    greatest(-1.0, least(1.0, 1.0 - (e.embedding <=> target_query_embedding)))::double precision as vector_similarity,
  ...
  order by f.score desc, e.embedding <=> target_query_embedding, k.id
  limit result_limit;
end
$$;
