...
import { assertBrowserUrlAllowed, scopeFingerprint, type BrowserDnsResolver } from "./policy";
...
const publicResolver: BrowserDnsResolver = async () => [{ address: "1.1.1.1", family: 4 }];
const privateResolver: BrowserDnsResolver = async () => [{ address: "10.10.2.3", family: 4 }];

describe("browser scope policy", () => {
  it("allows the exact authorized host and its subdomains when DNS is public", async () => {
    await expect(assertBrowserUrlAllowed("https://example.test/path", scope, publicResolver)).resolves.toBeInstanceOf(URL);
    await expect(assertBrowserUrlAllowed("https://app.example.test/path", scope, publicResolver)).resolves.toBeInstanceOf(URL);
  });

  it("blocks a host outside the attached scope before navigation", async () => {
    await expect(assertBrowserUrlAllowed("https://outside.test", scope, publicResolver)).rejects.toThrow("browser_target_out_of_scope");
  });
