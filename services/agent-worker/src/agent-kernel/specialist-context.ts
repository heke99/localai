...
export interface SpecialistPlanEntry {
  readonly role: SpecialistRole;
  readonly execution: "parallel-readonly" | "serial";
  readonly allowedToolClasses: readonly string[];
  readonly context: Readonly<Record<string, unknown>>;
}

function bounded(value: string, max: number) {
  return value.trim().slice(0, Math.max(256, max));
}
...
function requested(prompt: string, selectedSkills: readonly string[], pattern: RegExp) {
...
export function buildSpecialistPlan(input: {
  ...
  const request = bounded(input.prompt, input.maxPromptChars);
  const skills = input.selectedSkills.slice(0, 24);
  const common = { task, selectedSkills: skills };

  const entries: SpecialistPlanEntry[] = [{
    role: "planner",
    execution: "parallel-readonly",
    allowedToolClasses: ["none"],
    context: { ...common, request }
  }];

  if (input.task.requiresCurrentInformation || input.task.researchDepth !== "none") {
    entries.push({
      role: "researcher",
      execution: "parallel-readonly",
      allowedToolClasses: ["web_search", "web_fetch", "current_time"],
      context: { task, request }
    });
  }

  if (input.task.requiresRepository || requested(input.prompt, skills, /\b(code|coding|implementation|refactor|repository|repo|kod|implement|fix|bug)\b/i)) {
    entries.push({
      role: "coder",
      execution: "serial",
      allowedToolClasses: ["repository_read", "repository_write_after_checkpoint"],
      context: { ...common, request }
    });
  }

  if (requested(input.prompt, skills, /\b(performance|latency|throughput|benchmark|profil|optimi[sz]|prestanda|snabbare)\b/i)) {
    entries.push({
      role: "performance",
      execution: "parallel-readonly",
      allowedToolClasses: ["repository_read", "metrics_read", "benchmark_read"],
      context: { task, request }
    });
  }

  if (input.task.verificationRequirements.length > 0 || input.task.requiresRepository) {
    entries.push({
      role: "tester",
      execution: "parallel-readonly",
      allowedToolClasses: ["repository_read", "test_read", "ci_read"],
      context: { task, verificationRequirements: input.task.verificationRequirements, request }
    });
  }

  // Independent verifier is always included. It deliberately receives no selected-skill
  // conclusions so it can challenge the plan instead of echoing it.
  entries.push({
    role: "verifier",
    execution: "parallel-readonly",
    allowedToolClasses: ["evidence_read", "ci_read", "repository_read"],
    context: { task, request, verificationRequirements: input.task.verificationRequirements }
  });

  const priority: SpecialistRole[] = ["planner", "verifier", "researcher", "tester", "performance", "coder"];
  return entries
    .sort((a, b) => priority.indexOf(a.role) - priority.indexOf(b.role))
    .slice(0, Math.max(1, input.maxSubagents));
}
