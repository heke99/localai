...
if [[ -f "${ENV_FILE}" ]]; then
  ...
  READINESS_TOKEN="$(sed -n 's/^DIV3RSA_SECURITY_READINESS_TOKEN=//p' "${ENV_FILE}" | tail -n 1)"
fi
if [[ -z "${TOKEN}" ]]; then
...
DIV3RSA_SECURITY_READINESS_TOKEN=${READINESS_TOKEN}
...
upsert_env DIV3RSA_SECURITY_READINESS_TOKEN "${READINESS_TOKEN}" "${WORKER_ENV_FILE}"
