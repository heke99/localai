...
export function createVerificationPlan(task: TaskAnalysis, impact?: ImpactAnalysis): VerificationPlan {
  ...
  if (task.requiresRepository) {
    add(checks, "diff-review", true, "Repository work must inspect the final change set.");
    add(checks, "consequence-analysis", true, "Repository changes require impact analysis before completion.");
    add(checks, "typecheck", true, "Typed project changes must preserve type safety.");
    add(checks, "targeted-tests", true, "Affected tests are selected from the impact set.");
  ...
  if (task.requiresDatabase) add(checks, "database-invariants", true, "Database work must verify schema, policy and data invariants.");
  ...
  if (task.requiresSecurityReview) add(checks, "security-review", true, "High-risk or security-sensitive work requires a security review.");
  ...
  if (task.requiresDeployment) {
    add(checks, "build", true, "Deployments require a successful production build.");
  ...
  for (const hint of impact?.verificationHints ?? []) {
    ...
    if (supported) add(checks, supported, true, "Required by consequence analysis.");
  }
...
export async function executeVerificationPlan(plan: VerificationPlan, executor: VerificationExecutor, context: VerificationContext): Promise<VerificationReport> {
  ...
  for (const check of plan.checks) {
    ...
    try {
      result = await executor.run(check, context);
    } catch (error) {
      result = { kind: check.kind, status: "failed", summary: error instanceof Error ? error.message : "verification_executor_failed" };
    }
