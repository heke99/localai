...
import { discoverVercel, exchangeVercelCode, vercelAuthorizationUrl } from "./vercel-provider";
...
export function providerAuthorizationUrl(provider: ProviderKey, security: OAuthSecurity) {
  ...
  if (provider === "supabase") return supabaseAuthorizationUrl(security.state, security.codeChallenge);
...
export async function exchangeAndDiscover(provider: ProviderKey, code: string, codeVerifier: string | null, actorUserId: string) {
  ...
  const discovered = await discoverVercel(credential);
  return { ...discovered, credential };
}
