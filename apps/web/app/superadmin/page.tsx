...
import { enqueueOperation, grantAccess, reviewAccessRequest } from "./actions";
...
export default async function SuperadminPage() {
  ...
  return <main className="shell">
    ...
    <section className="admin-page">
      ...
      <section className="admin-section"><h3>Access review</h3>{requests?.length ? <div className="admin-list">{requests.map((request) => <article className="admin-row" key={request.id}><div><div className="eyebrow">{request.status}</div><strong>{request.name}</strong><small>{request.email} · {request.organization_name ?? "Independent"}</small><p>{request.use_case}</p></div><form className="row-actions"><input type="hidden" name="requestId" value={request.id}/><button formAction={reviewAccessRequest} name="decision" value="reviewing" className="button">Review</button><button formAction={grantAccess} className="button primary">Grant access</button><button formAction={reviewAccessRequest} name="decision" value="rejected" className="button danger">Reject</button></form></article>)}</div> : <p className="muted">No pending requests.</p>}</section>
