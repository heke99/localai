...
export async function setMembershipStatus(formData: FormData) {
  ...
  const { error } = await supabase.rpc("superadmin_set_membership_status", {
...
export async function createPolicySet(formData: FormData) {
  ...
  try {
    ...
    throw new Error("policy_conditions_must_be_json_object");
  }
  ...
  const { error } = await supabase.rpc("superadmin_create_policy_set", {
    target_organization_id: organizationId,
    target_key: key,
    target_effect: effect,
    target_action: action,
    target_resource_pattern: resourcePattern,
...
export async function setPolicyStatus(formData: FormData) {
  ...
  const { error } = await supabase.rpc("superadmin_set_policy_status", {
    target_policy_set_id: policySetId,
    target_status: status
  });
  if (error) throw new Error(error.message);
  revalidatePath("/superadmin");
...
export async function revokeLabAuthorization(formData: FormData) {
  const { supabase } = await requireSuperadmin();
  const authorizationId = uuid(formData.get("authorizationId"));
  const { error } = await supabase.rpc("superadmin_revoke_lab_authorization", {
...
export async function setGpuProviderEnabled(formData: FormData) {
  ...
  const { error } = await supabase.rpc("superadmin_set_gpu_provider_enabled", {
