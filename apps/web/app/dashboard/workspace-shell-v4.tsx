...
import { RunStreamPreview } from "./run-stream-preview";
...
type Integration = { id: string; provider: string; external_account_id: string; status: string; created_at: string; capabilities?: Record<string, boolean>; external_account_name?: string | null; metadata?: Record<string, unknown> };
...
export function WorkspaceShellV4({ workspaceId, workspaceName, displayName, email, isSuperadmin, snapshot }: { workspaceId: string; workspaceName: string; displayName: string; email: string; isSuperadmin: boolean; snapshot: WorkspaceSnapshot }) {
  ...
  async function disconnectProvider(provider: ProviderKey, connectionId: string) {
    ...
    try {
      ...
      setIntegrations((current) => current.map((item) => item.id === connectionId ? { ...item, status: "disconnected", external_account_name: null, metadata: {} } : item));
      setResources((current) => current.filter((resource) => resource.connection_id !== connectionId));
      setProjectResources((current) => current.filter((resource) => !disconnectedResourceIds.has(resource.resource_id)));
