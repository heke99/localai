...
function ResourceGrantCard({ resource, current, catalog, onSave }: { resource: Resource; current?: ProjectResource; catalog: Capability[]; onSave: (resourceId: string, capabilities: string[], enabled: boolean) => Promise<void> }) {
  ...
  useEffect(() => {
    setSelected(current?.capabilities ?? []);
    setEnabled(current?.enabled ?? false);
  }, [current?.enabled, current?.resource_id, JSON.stringify(current?.capabilities ?? [])]);

  async function save() {
    setSaving(true);
    try { await onSave(resource.id, selected, enabled); }
    finally { setSaving(false); }
  }

  return <article className={styles.permissionCard}>
    <div className={styles.permissionHead}>
      <span className={styles.providerBadge}>{providerMark(resource.provider)}</span>
      <div className={styles.permissionIdentity}><strong>{resource.display_name}</strong><small>{providerName(resource.provider)} · {resourceKind(resource)}</small></div>
      <label className={styles.switch}><input type="checkbox" checked={enabled} onChange={(event) => setEnabled(event.target.checked)} /><span>{enabled ? "Aktiv" : "Av"}</span></label>
    </div>
    <div className={styles.permissionOptions}>{available.map((cap) => <label key={cap.capability} className={`${styles.permissionOption} ${styles[`risk_${cap.risk}`]}`}>
      <input type="checkbox" checked={selected.includes(cap.capability)} disabled={!enabled} onChange={(event) => setSelected((currentCaps) => event.target.checked ? [...new Set([...currentCaps, cap.capability])] : currentCaps.filter((item) => item !== cap.capability))} />
      <span><strong>{cap.label}</strong><small>{cap.risk}</small></span>
    </label>)}</div>
...
export function WorkspaceShellV3({ workspaceId, workspaceName, displayName, email, isSuperadmin, snapshot }: { workspaceId: string; workspaceName: string; displayName: string; email: string; isSuperadmin: boolean; snapshot: WorkspaceSnapshot }) {
  ...
  const [integrations] = useState<Integration[]>(snapshot.integrations ?? []);
  ...
  const [selectedProjectId, setSelectedProjectId] = useState<string | null>(() => (snapshot.conversations ?? []).find((item) => item.mode === "chat")?.project_id ?? null);
  ...
  function switchMode(mode: Mode) {
    ...
    setError(null);
    const candidate = conversations.find((conversation) => conversation.mode === mode && conversation.project_id === selectedProjectId);
  ...
  function toggleResource(resourceId: string) {
    const next = selectedResourceIds.includes(resourceId) ? selectedResourceIds.filter((id) => id !== resourceId) : [...new Set([...selectedResourceIds, resourceId])];
    void persistConversationResources(next);
  }
  ...
  async function submitPrompt() {
    ...
    try {
      ...
      setMessages((current) => [...current, { id: `local-${Date.now()}`, role: "user", content: { text }, created_at: new Date().toISOString() }]);
      setPrompt("");
      ...
      setError(caught instanceof Error && /resource|access|permission/.test(caught.message) ? "En vald resurs eller rättighet är inte längre tillgänglig. Uppdatera resursvalet och försök igen." : "Uppgiften kunde inte startas. Försök igen.");
    } finally { setBusy(false); }
  }
  ...
  function connectProvider(provider: ProviderKey) {
    if (busy) return;
    setBusy(true); setError(null);
    const returnPath = "/dashboard?section=integrations";
    window.location.assign(`/api/integrations/${provider}/connect?workspaceId=${encodeURIComponent(workspaceId)}&returnPath=${encodeURIComponent(returnPath)}`);
  }
  ...
  async function saveProjectResource(resourceId: string, capabilities: string[], enabled: boolean) {
    ...
    setProjectResources((current) => {
      const next: ProjectResource = { project_id: manageProjectId, resource_id: resourceId, enabled, connection_id: resource.connection_id, provider: resource.provider, resource_type: resource.resource_type, external_resource_id: resource.external_resource_id, display_name: resource.display_name, metadata: resource.metadata, capabilities: body.binding?.capabilities ?? capabilities };
      return [next, ...current.filter((item) => !(item.project_id === manageProjectId && item.resource_id === resourceId))];
    });
  }
  ...
  return <div className={`${styles.shell} ${showChat ? "" : styles.shellWide}`}>
    <aside className={styles.rail}>
      <div className={styles.brandArea}>
        <Link href="/dashboard" className={styles.brand}>DIV3RSA</Link>
        <button className={styles.newChatButton} type="button" onClick={() => startNewChat(activeMode, null)}><span>＋</span> Ny chatt</button>
      </div>
      <nav className={styles.nav} aria-label="Huvudnavigation">
        {(Object.keys(modeMeta) as Mode[]).map((mode) => <button key={mode} type="button" className={`${styles.navButton} ${activeSection === mode ? styles.navButtonActive : ""}`} onClick={() => switchMode(mode)}><span className={styles.navGlyph}>{modeMeta[mode].short}</span><span>{modeMeta[mode].label}</span></button>)}
        <div className={styles.navDivider}/>
        <button className={`${styles.navButton} ${activeSection === "projects" ? styles.navButtonActive : ""}`} onClick={() => setActiveSection("projects")}><span className={styles.navGlyph}>P</span><span>Projekt</span></button>
        <button className={`${styles.navButton} ${activeSection === "integrations" ? styles.navButtonActive : ""}`} onClick={() => setActiveSection("integrations")}><span className={styles.navGlyph}>+</span><span>Integrationer</span></button>
        <button className={`${styles.navButton} ${activeSection === "settings" ? styles.navButtonActive : ""}`} onClick={() => setActiveSection("settings")}><span className={styles.navGlyph}>⚙</span><span>Inställningar</span></button>
      </nav>
    ...
    {showChat && <aside className={styles.chatSidebar}>
      ...
      <div className={styles.sidebarScroll}>
        <section className={styles.sidebarSection}>
          <div className={styles.sidebarTitle}><span>Fristående</span><button type="button" onClick={() => startNewChat(activeMode, null)}>＋</button></div>
          {standaloneChats.map((conversation) => <button key={conversation.id} className={`${styles.chatRow} ${selectedConversationId === conversation.id ? styles.chatRowActive : ""}`} onClick={() => selectConversation(conversation)}><span>{conversation.title || "Ny chatt"}</span><small>{relative(conversation.last_message_at || conversation.updated_at)}</small></button>)}
          {!standaloneChats.length && <button className={styles.emptyRow} onClick={() => startNewChat(activeMode, null)}>Starta en fristående chatt</button>}
        </section>

        <section className={styles.sidebarSection}>
          <div className={styles.sidebarTitle}><span>Projekt</span><button type="button" onClick={() => setActiveSection("projects")}>＋</button></div>
          {projects.map((project) => <div className={styles.projectTree} key={project.id}>
            <div className={`${styles.projectRow} ${selectedProjectId === project.id ? styles.projectRowActive : ""}`}>
              <button className={styles.projectOpen} onClick={() => openProject(project.id, activeMode)}><span className={styles.projectDot}/><span>{project.name}</span></button>
              <button className={styles.projectAddChat} title={`Ny chatt i ${project.name}`} onClick={() => startNewChat(activeMode, project.id)}>＋</button>
            </div>
            {selectedProjectId === project.id && <div className={styles.projectChats}>{selectedProjectChats.map((conversation) => <button key={conversation.id} className={`${styles.chatRow} ${selectedConversationId === conversation.id ? styles.chatRowActive : ""}`} onClick={() => selectConversation(conversation)}><span>{conversation.title || "Ny chatt"}</span><small>{relative(conversation.last_message_at || conversation.updated_at)}</small></button>)}{!selectedProjectChats.length && <button className={styles.emptyRow} onClick={() => startNewChat(activeMode, project.id)}>Skapa första chatten</button>}</div>}
          </div>)}
          {!projects.length && <button className={styles.emptyRow} onClick={() => setActiveSection("projects")}>Skapa ditt första projekt</button>}
        </section>
      </div>
    </aside>}
    ...
    <main className={styles.main}>
      {showChat && <>
        <header className={styles.chatHeader}>
          <div><div className={styles.breadcrumb}>{selectedProject ? selectedProject.name : "Fristående"} <span>/</span> {modeMeta[activeMode].label}</div><h1>{selectedConversation?.title && selectedConversation.title !== "Ny chatt" ? selectedConversation.title : "Ny chatt"}</h1></div>
          <button className={styles.headerNewChat} type="button" onClick={() => startNewChat(activeMode, selectedProjectId)}>＋ Ny chatt</button>
        </header>
        ...
        <section className={styles.chatCanvas}>
          {loadingConversation ? <div className={styles.centerState}>Laddar chatt…</div> : messages.length ? <div className={styles.messageStream}>{messages.map((message) => <article key={message.id} className={`${styles.message} ${message.role === "user" ? styles.userMessage : styles.assistantMessage}`}><div className={styles.messageMeta}>{message.role === "user" ? "Du" : "DIV3RSA"}</div><div className={styles.messageBody}>{messageText(message.content)}</div></article>)}</div> : <div className={styles.emptyChat}>
            <div className={styles.emptyIcon}>{modeMeta[activeMode].short}</div>
            <h2>{selectedProject ? `Arbeta i ${selectedProject.name}` : "Vad vill du göra?"}</h2>
            <p>{modeMeta[activeMode].description}</p>
            <button className={styles.emptyResourceButton} type="button" onClick={() => setResourcePickerOpen(true)}>＋ Lägg till repo eller resurs</button>
            <small>Välj GitHub, Supabase, Vercel eller framtida plugins. Agenten får deras kontext och verktyg i just den här chatten.</small>
          </div>}
          {run && <div className={styles.runBar}><span className={`${styles.runDot} ${terminalStatuses.has(run.status) ? styles.runDotIdle : ""}`}/><strong>{run.status}</strong><span>{run.model_alias}</span>{run.failure_code && <span className={styles.runError}>{run.failure_code}</span>}{!terminalStatuses.has(run.status) && <button onClick={cancelRun}>Stoppa</button>}</div>}
        </section>

        <footer className={styles.composerArea}>
          <div className={styles.contextLine}><span className={styles.locationChip}>{selectedProject?.name ?? "Fristående chatt"}</span>{selectedResources.map((resource) => <button type="button" className={styles.resourceChip} key={resource.id} onClick={() => toggleResource(resource.id)} title="Ta bort resurs"><span className={styles.chipMark}>{providerMark(resource.provider)}</span><span>{resource.display_name}</span><span className={styles.chipClose}>×</span></button>)}</div>
          <div className={styles.composer}>
            <button className={`${styles.addResourceButton} ${selectedResources.length ? styles.addResourceButtonActive : ""}`} type="button" title="Lägg till repo, projekt eller plugin" onClick={() => setResourcePickerOpen(true)}>＋</button>
            <textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void submitPrompt(); } }} placeholder={modeMeta[activeMode].placeholder} disabled={busy} rows={2}/>
            <button className={styles.sendButton} type="button" onClick={() => void submitPrompt()} disabled={busy || !prompt.trim()}>{busy ? "…" : "↑"}</button>
          </div>
          <div className={styles.composerHelp}><span>{selectedResources.length ? `${selectedResources.length} aktiv${selectedResources.length === 1 ? " resurs" : "a resurser"}` : "Ingen extern resurs vald"}</span><span>Enter för att skicka · Shift+Enter för ny rad</span></div>
          {error && <p className={styles.error}>{error}</p>}
        </footer>
      </>}

      {activeSection === "projects" && <section className={styles.page}>
        <div className={styles.pageHeader}><div><span className={styles.kicker}>Arbetsytor</span><h1>Projekt</h1><p>Samla chattar efter produkt eller uppdrag. Projekt är valfria — du kan alltid starta en fristående chatt.</p></div><button className={styles.primaryButton} onClick={() => setProjectFormOpen(true)}>＋ Nytt projekt</button></div>
        {projectFormOpen && <form className={styles.projectForm} onSubmit={createProject}><label>Projektnamn<input value={projectName} onChange={(event) => setProjectName(event.target.value)} maxLength={120} required autoFocus/></label><label>Beskrivning<textarea value={projectDescription} onChange={(event) => setProjectDescription(event.target.value)} maxLength={2000} rows={3}/></label><div className={styles.formActions}><button type="button" onClick={() => setProjectFormOpen(false)}>Avbryt</button><button className={styles.primaryButton} disabled={busy}>Skapa projekt</button></div></form>}
        <div className={styles.projectGrid}>{projects.map((project) => { const chats = conversations.filter((chat) => chat.project_id === project.id); const bound = projectResources.filter((resource) => resource.project_id === project.id && resource.enabled); return <article className={styles.projectCard} key={project.id}><div className={styles.projectCardTop}><span className={styles.projectMark}>{project.name.slice(0,2).toUpperCase()}</span><div><h2>{project.name}</h2><p>{project.description || "Inga anteckningar ännu."}</p></div></div><div className={styles.projectStats}><span><strong>{chats.length}</strong> chattar</span><span><strong>{bound.length}</strong> resurser</span><span><strong>{bound.filter((resource) => resource.provider === "github").length}</strong> repos</span></div><div className={styles.projectActions}><button className={styles.primaryButton} onClick={() => startNewChat("chat", project.id)}>＋ Ny chatt</button><button onClick={() => openProject(project.id, "chat")}>Öppna projekt</button></div></article>; })}</div>
        {!projects.length && <div className={styles.bigEmpty}><h2>Inga projekt ännu</h2><p>Du kan skapa ett projekt för att samla flera chattar, eller fortsätta arbeta fristående.</p><div><button className={styles.primaryButton} onClick={() => setProjectFormOpen(true)}>Skapa projekt</button><button onClick={() => startNewChat("chat", null)}>Fristående chatt</button></div></div>}
        {error && <p className={styles.error}>{error}</p>}
      </section>}

      {activeSection === "integrations" && <section className={styles.page}>
        <div className={styles.pageHeader}><div><span className={styles.kicker}>Anslutningar</span><h1>Integrationer</h1><p>Anslut konton en gång. I varje chatt väljer du sedan exakt vilket repo eller projekt agenten ska arbeta mot.</p></div></div>
        <div className={styles.integrationGrid}>{(["github", "supabase", "vercel"] as ProviderKey[]).map((provider) => { const connection = integrations.find((item) => item.provider === provider && ["connected", "active", "ready", "pending"].includes(item.status)); const connected = Boolean(connection && ["connected", "active", "ready"].includes(connection.status)); return <article className={styles.integrationCard} key={provider}><div className={styles.integrationIdentity}><span className={styles.providerBadge}>{providerMark(provider)}</span><div><h2>{providerName(provider)}</h2><p>{providerMeta[provider].description}</p></div></div><div className={styles.integrationAction}><span className={`${styles.status} ${connected ? styles.statusOn : ""}`}>{connected ? "Ansluten" : connection?.status === "pending" ? "Påbörjad" : "Inte ansluten"}</span><button disabled={busy || connected} onClick={() => connectProvider(provider)}>{connected ? "Klar" : connection?.status === "pending" ? "Fortsätt" : "Anslut"}</button></div></article>; })}</div>

        <section className={styles.advancedAccess}>
          <div className={styles.advancedHeader}><div><span className={styles.kicker}>Avancerad projektåtkomst</span><h2>Utökade actions</h2><p>När en resurs väljs i en chatt läggs tillgänglig read/write-access till automatiskt. Här kan du finjustera eller ge destructive/sensitive actions för ett specifikt projekt.</p></div><select value={manageProjectId} onChange={(event) => setManageProjectId(event.target.value)}><option value="">Välj projekt</option>{projects.map((project) => <option value={project.id} key={project.id}>{project.name}</option>)}</select></div>
          {manageProject ? <div className={styles.permissionGrid}>{connectedResources.map((resource) => <ResourceGrantCard key={`${manageProject.id}:${resource.id}`} resource={resource} current={projectResources.find((item) => item.project_id === manageProject.id && item.resource_id === resource.id)} catalog={catalog} onSave={saveProjectResource}/>)}</div> : <div className={styles.bigEmpty}><h2>Välj ett projekt</h2><p>Fristående chattar använder säker read/write-basåtkomst. Utökad åtkomst hanteras på projektnivå.</p></div>}
        </section>
        {error && <p className={styles.error}>{error}</p>}
      </section>}

      {activeSection === "settings" && <section className={styles.page}>
        <div className={styles.pageHeader}><div><span className={styles.kicker}>Konto</span><h1>Inställningar</h1><p>Konto, arbetsyta och systemgenvägar.</p></div></div>
        <div className={styles.settingsGrid}><article><span>Konto</span><strong>{displayName}</strong><small>{email}</small><Link href="/auth/set-password?mode=change">Ändra lösenord</Link></article><article><span>Arbetsyta</span><strong>{workspaceName}</strong><small>{projects.length} projekt · {connectedResources.length} synkade resurser</small><button onClick={() => setActiveSection("integrations")}>Hantera integrationer</button></article>{isSuperadmin && <article><span>System</span><strong>Control Center</strong><small>Modeller, GPU, policies och drift.</small><Link href="/superadmin">Öppna Control Center</Link></article>}<article><span>Session</span><strong>Logga ut</strong><small>Avsluta den aktiva sessionen.</small><form action="/auth/signout" method="post"><button>Logga ut</button></form></article></div>
      </section>}
    </main>

    {resourcePickerOpen && <div className={styles.resourceOverlay} onMouseDown={() => setResourcePickerOpen(false)}>
      <section className={styles.resourcePanel} onMouseDown={(event) => event.stopPropagation()} aria-label="Välj resurser">
        <div className={styles.resourcePanelHeader}><div><span className={styles.kicker}>Aktiv arbetskontext</span><h2>Lägg till resurs</h2><p>Välj vad agenten ska förstå och kunna arbeta mot i den här chatten.</p></div><button className={styles.closeButton} onClick={() => setResourcePickerOpen(false)}>×</button></div>
        <div className={styles.resourceSearch}><span>⌕</span><input autoFocus value={resourceSearch} onChange={(event) => setResourceSearch(event.target.value)} placeholder="Sök repo, Supabase-projekt, Vercel-projekt…"/></div>
        <div className={styles.resourceList}>{resourceGroups.map(([provider, providerResources]) => <div className={styles.resourceGroup} key={provider}><div className={styles.resourceGroupTitle}><span className={styles.providerBadge}>{providerMark(provider)}</span><strong>{providerName(provider)}</strong><small>{providerResources.length}</small></div>{providerResources.map((resource) => { const selected = selectedResourceIds.includes(resource.id); const projectBinding = selectedProjectId ? projectResources.find((item) => item.project_id === selectedProjectId && item.resource_id === resource.id) : undefined; return <button type="button" key={resource.id} className={`${styles.resourceOption} ${selected ? styles.resourceOptionSelected : ""}`} onClick={() => toggleResource(resource.id)}><span className={styles.resourceCheck}>{selected ? "✓" : ""}</span><span className={styles.resourceOptionText}><strong>{resource.display_name}</strong><small>{resourceKind(resource)} · {projectBinding?.capabilities?.length ? `${projectBinding.capabilities.length} anpassade actions` : "säker read/write-bas"}</small></span><span className={styles.resourceArrow}>{selected ? "Vald" : "Lägg till"}</span></button>; })}</div>)}{!resourceGroups.length && <div className={styles.resourceEmpty}><h3>{connectedResources.length ? "Ingen matchning" : "Inga synkade resurser ännu"}</h3><p>{connectedResources.length ? "Prova ett annat sökord." : "Anslut GitHub, Supabase eller Vercel för att börja arbeta direkt mot externa system."}</p></div>}</div>
        <div className={styles.resourcePanelFooter}><button onClick={() => { setResourcePickerOpen(false); setActiveSection("integrations"); }}>Hantera integrationer</button><button className={styles.primaryButton} onClick={() => setResourcePickerOpen(false)}>Klar · {selectedResourceIds.length} valda</button></div>
      </section>
    </div>}
  </div>;
}
