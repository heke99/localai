...
export async function POST(request: Request, context: { params: Promise<{ conversationId: string }> }) {
  ...
  const { data, error } = await rpc.rpc<Record<string, unknown>>("set_conversation_resources", { target_conversation_id: conversationId, target_resource_ids: resourceIds });
  if (error) {
    const denied = /authentication_required|conversation_access_denied|resource_not_available|project_required_for_integration_resources/.test(error.message);
